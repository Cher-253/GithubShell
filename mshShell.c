/*
    Name: Cherryl Maria Bibin
    Student ID: 1001759133
*/
...
int main()
{
    ...
    while( 1 )
    {
        ...
        //if user enters !n
        else if(token[0][0]=='!')//using ' ' because it is a character
        {
            if(history_filled == 1)//history array is full
            { 
                //in order to change the number from its ASCII value to the number 
                //subtract ASCII value of 0 from ASCII value of n  
                int newNum = token[0][1] - '0'; 
                // printf("the command number is : %c\n\n",newNum);
                // printf("The command at this number is : %s\n\n",history[newNum]);
                cmd_str = history[newNum]; //storing it into cmd str 
                //storing the new command string that !n gave into a char pointer
                newCommandString = cmd_str;
            }
            else
            {
                int newNum = token[0][1] - '0';
